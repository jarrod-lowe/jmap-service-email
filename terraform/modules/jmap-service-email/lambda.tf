...
# Lambda function for SearchSnippet/get
resource "aws_lambda_function" "searchsnippet_get" {
  function_name = "${local.name_prefix}-searchsnippet-get"
  role          = aws_iam_role.lambda_execution.arn
  handler       = "bootstrap"
  runtime       = "provided.al2023"
  architectures = ["arm64"]
  ...
  filename         = "${path.module}/../../../build/searchsnippet-get/lambda.zip"
  source_code_hash = filebase64sha256("${path.module}/../../../build/searchsnippet-get/lambda.zip")
  ...
  environment {
    variables = {
      ...
      # OpenTelemetry SDK Configuration
      OTEL_SERVICE_NAME           = "${local.name_prefix}-searchsnippet-get"
      OTEL_TRACES_SAMPLER         = "always_on"
      OTEL_PROPAGATORS            = "tracecontext,baggage,xray"
      OTEL_EXPORTER_OTLP_ENDPOINT = "http://localhost:4317"
      OTEL_EXPORTER_OTLP_PROTOCOL = "grpc"
  ...
  depends_on = [
    aws_cloudwatch_log_group.searchsnippet_get,
    aws_iam_role_policy_attachment.lambda_basic,
    aws_iam_role_policy_attachment.lambda_xray,
    aws_iam_role_policy_attachment.dynamodb_email_data
  ]
...
# Permission for jmap-api Lambda to invoke searchsnippet-get function
resource "aws_lambda_permission" "allow_jmap_core_searchsnippet_get" {
  statement_id  = "AllowJMAPCoreInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.searchsnippet_get.function_name
  principal     = "lambda.amazonaws.com"
  source_arn    = "arn:aws:lambda:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:function:jmap-api-*"
}
