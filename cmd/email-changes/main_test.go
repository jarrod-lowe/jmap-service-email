...
func TestHandler_BasicChanges(t *testing.T) {
	mockRepo := &mockStateRepository{
		...
		queryChangesFunc: func(ctx context.Context, accountID string, objectType state.ObjectType, sinceState int64, maxChanges int) ([]state.ChangeRecord, error) {
			return []state.ChangeRecord{
				{ObjectID: "email-1", ChangeType: state.ChangeTypeCreated, State: 4},
				{ObjectID: "email-2", ChangeType: state.ChangeTypeCreated, State: 5},
			}, nil
		},
	}
	...
	// Should return Email/changes response
	if response.MethodResponse.Name != "Email/changes" {
		t.Errorf("Name = %q, want %q", response.MethodResponse.Name, "Email/changes")
	}
	...
	// Should have created list with two emails
	...
	if len(created) != 2 {
		t.Errorf("created length = %d, want 2", len(created))
	}
...
func TestHandler_UpdatedAndDestroyed(t *testing.T) {
	mockRepo := &mockStateRepository{
		getCurrentStateFunc: func(ctx context.Context, accountID string, objectType state.ObjectType) (int64, error) {
			return 10, nil
		},
		...
		queryChangesFunc: func(ctx context.Context, accountID string, objectType state.ObjectType, sinceState int64, maxChanges int) ([]state.ChangeRecord, error) {
			return []state.ChangeRecord{
				{ObjectID: "email-1", ChangeType: state.ChangeTypeUpdated, State: 8},
				{ObjectID: "email-2", ChangeType: state.ChangeTypeDestroyed, State: 9},
				{ObjectID: "email-3", ChangeType: state.ChangeTypeCreated, State: 10},
			}, nil
		},
	}
	...
	// Check created
	created, ok := response.MethodResponse.Args["created"].([]string)
	if !ok || len(created) != 1 || created[0] != "email-3" {
		t.Errorf("created = %v, want [email-3]", created)
	}

	// Check updated
	updated, ok := response.MethodResponse.Args["updated"].([]string)
	if !ok || len(updated) != 1 || updated[0] != "email-1" {
		t.Errorf("updated = %v, want [email-1]", updated)
	}

	// Check destroyed
	destroyed, ok := response.MethodResponse.Args["destroyed"].([]string)
	if !ok || len(destroyed) != 1 || destroyed[0] != "email-2" {
		t.Errorf("destroyed = %v, want [email-2]", destroyed)
	}
}
...
func TestHandler_NoChanges(t *testing.T) {
	...
	if response.MethodResponse.Name != "Email/changes" {
		t.Fatalf("Name = %q, want %q", response.MethodResponse.Name, "Email/changes")
	}
		...
		queryChangesFunc: func(ctx context.Context, accountID string, objectType state.ObjectType, sinceState int64, maxChanges int) ([]state.ChangeRecord, error) {
			...
			for i := 0; i < 10; i++ {
				changes[i] = state.ChangeRecord{
					ObjectID:   "email-" + string(rune('a'+i)),
					ChangeType: state.ChangeTypeCreated,
					State:      int64(i + 1),
				}
			}
			return changes, nil
...
func TestHandler_CreatedThenDestroyed_OmitsFromResults(t *testing.T) {
	mockRepo := &mockStateRepository{
		getCurrentStateFunc: func(ctx context.Context, accountID string, objectType state.ObjectType) (int64, error) {
			return 10, nil
		},
		...
		queryChangesFunc: func(ctx context.Context, accountID string, objectType state.ObjectType, sinceState int64, maxChanges int) ([]state.ChangeRecord, error) {
			// email-1 was created then destroyed - should be omitted entirely
			return []state.ChangeRecord{
				{ObjectID: "email-1", ChangeType: state.ChangeTypeCreated, State: 8},
				{ObjectID: "email-1", ChangeType: state.ChangeTypeDestroyed, State: 9},
				{ObjectID: "email-2", ChangeType: state.ChangeTypeCreated, State: 10},
			}, nil
		},
	}
	...
	// email-1 should NOT appear in any list (created then destroyed)
	created, ok := response.MethodResponse.Args["created"].([]string)
	if !ok {
		t.Fatalf("created should be []string, got %T", response.MethodResponse.Args["created"])
	}
	...
	for _, id := range created {
		if id == "email-1" {
			t.Error("email-1 should not be in created list")
		}
	}
	for _, id := range destroyed {
		if id == "email-1" {
			t.Error("email-1 should not be in destroyed list")
		}
	}

	// email-2 should be in created
	if len(created) != 1 || created[0] != "email-2" {
		t.Errorf("created = %v, want [email-2]", created)
	}
}
